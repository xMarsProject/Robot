...
int bot_init()
{
    ...
    bot_wheel_l=0;
    bot_wheel_r=0;
    wiringPiISR(bot_left_encoded_wheel,INT_EDGE_RISING,&interupt_p0); // set interupt to count  the ticks of the left wheel
    wiringPiISR(bot_right_encoded_wheel,INT_EDGE_RISING,&interupt_p21); // set interupt to count the ticks of the right wheel

    //Left sound sensor
    pinMode(bot_left_sound_sensor, INPUT);
    return 1;
}
...
/**********************************************************************************************************
 Sound sensor Management
***********************************************************************************************************/

int bot_get_left_sound(void)
{
   return analogRead(bot_left_sound_sensor);
}
